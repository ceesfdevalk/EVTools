...
#' @title FitGW_Weibull
#' 
#' @description Fit a Generalised Weibull (GW) upper tail with positive tail index to the sample X and estimate quantiles
#' 
...
#' 
#' @usage Value <- FitGW_Weibull(X, p, N= 0, r11= 1, fixedpar= NULL, l0= NULL, sigma= 1, XId= '')
#' 
...
#'  comparison to known shift-invariant estimators. 
#'  
...
FitGW_iHill <- function(X, p, N, r11, fixedpar, l0, sigma, XId) {
  ...
  if (n > 0) {
    ...
    ind <- which(k> 2 & X0[k]> 0 & l> 0  & k< n & k>= l)
    if (length(ind)> 0) {
      k <- k[ind]
      l <- l[ind]
    } else {
    ...
    if (nl> 0) {
      ...
      if (length(theta0)== 0) {
        ...
        # Simple estimator of Weibull index (nondimensional curvature)
        ...
        thetaStd= sqrt(r11value/k)*theta
