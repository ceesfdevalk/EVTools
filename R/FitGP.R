...
#' 
#' @usage Value <- FitGP(X, method= "FitGW_iHilli", p= NULL, N= 0, r11= 1, fixedpar= NULL, l0= NULL, sigma= Inf, metadata= NULL)
#' 
...
#' 
#' @export
FitGP <- function(X, method, p, N, r11, fixedpar, l0, sigma, metadata) {
  
  # Handle arguments
  if (missing(method)) {method <- "FitGP_Mom"}
  if (missing(p)) {p <- NULL}
  if (missing(N)) {N <- 0} 
  ...
  known <- grep(method, c("FitGP_Mom","FitGP_MLE2"))
  if (length(known)< 1) {
    stop("Specified method is unknown to FitGP.R")
  }
  
  estimates <- get(method)(X, p, N, r11, fixedpar, l0, sigma, metadata)
}
